---
title: "Compositional dynamics: ResourceSharers and Machines"
author: "Simon Frost"
date: "`r Sys.Date()`"
output:
  rmarkdown::html_vignette:
    toc: true
vignette: >
  %\VignetteIndexEntry{Compositional dynamics: ResourceSharers and Machines}
...
neatly into the Petri net framework — especially when:
- Rate laws are nonlinear or involve sums like total population *N*
- The system includes **delay differential equations** (DDEs)
- You want **directed** signal flow (inputs and outputs)
...
|-----------|------------|-------------|
| **Petri nets** | `oapply()` via UWD | Mass-action kinetics, stoichiometric models |
| **ResourceSharers** | `oapply_dynam()` via UWD | General ODEs with shared state |
| **Machines** | `oapply_machine()` via DWD | Directed signal flow, control systems |
